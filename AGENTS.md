...
- Design system: one dark "Event Log" theme using Fraunces, Inter, and JetBrains Mono. Palette custom properties (`--bg`, `--surface`, `--hairline`, `--text`, `--text-muted`, `--verified`) live under `:root` in `styles.css`. Reserve `--verified` (`#3DDC84`) for verified and impact markers; use neutral colors for all other decoration. The `@media print` rules provide the light recruiter/PDF variant.
